...
- **Placement:** decided once, when it opens, and never revisited. With `position: cursor` (the
  ...
  window, and with `position: last` it opens where a pinned preview was last dropped (`E64`); in
  both the pointer is *outside* it, so the press that pins it is the one on the picture
...
| `T40` | `S10`/`S25` | Drag a pinned window and let go (Ctrl held: no snapping) | It snaps 8 px to the window's corners, edge middles and centre on the axes the drag moved. In a tour the drop becomes the slideshow's spot; under `position: last` it becomes where previews open; otherwise nothing is remembered (`E64`) |
