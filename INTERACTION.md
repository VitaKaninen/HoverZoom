...
| `T35` | `S25` | Get within 10 of the end of the list, or press ▶ at the end | The page hops to the bottom and straight back (a frame or two), so a lazy feed loads its next batch; the counter grows and the tour carries on. If the hop loads nothing, one stay at the bottom is tried per page (`excLong`). Once an excursion returns nothing, it stops trying (`E58`) |
