...
Describes `Hover-Zoom.user.js` **v0.144.0**.
...
| `P4` | Not a media/plugin tag, and no **player** on this page covering it: not inside a `<video>`, not sitting in a **video surface** (a laid-out player's rectangle, or the player box derived from it), and no **dormant** (not laid out) player in it or within three ancestors — a laid-out player beside it is beside it, and geometry alone decides whether it covers it. A `<video>` that is muted, controls-less, looping or autoplaying, and under a minute long is an animated picture rather than a player — it is none of those things (`E12`). No setting — a player on the page always wins | — |
...
- **Placement:** decided once, when it opens, and never revisited. With `position: cursor` (the
  ...
  overrode it, so the gap could never survive.) With `position: last` it opens where a pinned
  preview was last dropped — the centre until one is (`E64`) — and the pointer is *outside* it, so the press that pins it is the one on the picture
...
| `T35` | `S25` | Every landing on a learned site (`E58`), either way; elsewhere the page never moves | The page scrolls so the picture sits near the top of the screen (only when it has left the 10–40 % band), so the page shows what comes next, as a reader scrolls. When the bottom comes on screen the page's own loader fires and the new batch joins the slideshow, measured ~14 pictures before the end. ▶ on the last picture with the bottom still off screen scrolls on one screen instead of ending |
...
| `K7` | Releasing the hotkey | Modifier mode only. Pressing it again over the same picture — or twice quickly anywhere on it — **pins it**; a press on a pinned window closes it without suppressing (`E66`). |
| `K8` | Any mouse button held down | Suppresses *new* windows rather than closing the current one. Unconditional since v0.39.0. |
| `K9` | Pressing the hotkey, `hover` mode without toggle | Closes whatever is open, **pinned included**, like a right press (suppresses). Not while any button is held: the hotkey is then the drag's (`E66`). |
...
| `E30` | A preview that opened away from the pointer is pinned from the picture, not from the window — in any mode | [`docs/VIEWER.md`](docs/VIEWER.md) |
...
| `E58` | A lazy page loads more only because the slideshow scrolls the page along ahead of itself (`T35`) — never a jump to the bottom, which skipped the middle of feeds that mount batches near the viewport (Google in Firefox); removed v0.144.0. Only on sites where the user's own scrolling has been seen to add pictures (`scrollSites`, v0.141.0; forgotten after 5 empty page loads in a row, v0.143.0). A page that loaded nothing is not asked again until it has grown | [`docs/TOUR.md`](docs/TOUR.md) |
...
| `E67` | The slideshow's ends are live: ◀/← on the first picture and ▶/→ on the last end it; ▶ first asks for more (the page scrolled on, then the next page) and ends only when `tourExhausted()`. With nothing open ← starts at the last picture loaded. A held key never ends it | [`docs/TOUR.md`](docs/TOUR.md) |
